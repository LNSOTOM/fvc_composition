...
with rasterio.open(mask_path) as src_mask:
    mask = src_mask.read(1).astype('int')  # Read the mask (single-band)

# Define unique classes in the mask
classes = np.unique(mask)
classes = classes[classes != 0]  # Exclude 0 if it represents "no data"
...
for cls in classes:
    ...
    reflectance_values = [bands[band_idx][class_mask] for band_idx in range(bands.shape[0])]
    
    # Calculate mean and confidence intervals per band
    means = [np.mean(band) for band in reflectance_values]
    std_devs = [np.std(band) for band in reflectance_values]
    n = len(reflectance_values[0])  # Number of pixels in the class
    
    # Confidence intervals
    t_value = t.ppf((1 + confidence_level) / 2, df=n - 1)
    cis = [t_value * (std / np.sqrt(n)) for std in std_devs]
    
    mean_reflectance[cls] = means
    confidence_intervals[cls] = cis

# Plotting mean reflectance and confidence intervals
bands = [f"Band {i+1}" for i in range(bands.shape[0])]
x = np.arange(len(bands))

plt.figure(figsize=(12, 8))
for cls in classes:
    means = mean_reflectance[cls]
    cis = confidence_intervals[cls]
    plt.plot(x, means, label=f'Class {cls}')
...
for cls_name in mean_reflectance:
    ...
    plt.plot(x, means, label=f'Class {cls_name}')
    plt.fill_between(x, np.array(means) - np.array(cis), np.array(means) + np.array(cis), alpha=0.2)
...
# Define wavelengths (x-axis values) for the 5 bands
wavelengths = np.linspace(400, 900, bands.shape[0])  # 400 nm to 900 nm evenly spaced
...
for cls_name in mean_reflectance:
    means = mean_reflectance[cls_name]
    cis = confidence_intervals[cls_name]
    plt.plot(wavelengths, means, label=f'Class {cls_name}')
    plt.fill_between(wavelengths, np.array(means) - np.array(cis), np.array(means) + np.array(cis), alpha=0.2)

plt.xticks(wavelengths, labels=[f"{int(wl)} nm" for wl in wavelengths])
...
# Define wavelengths (x-axis values)
wavelengths_full = [400, 500, 600, 700, 800, 900]  # Full range of wavelengths
wavelengths_used = [400, 500, 600, 700, 800]  # Only wavelengths corresponding to 5 bands
...
for cls, cls_name in class_labels.items():
    ...
    # Extend to match full wavelength range (pad missing values with NaN for 900 nm)
    means_full = means + [np.nan]  # Add NaN for the extra wavelength
    cis_full = cis + [np.nan]      # Add NaN for the extra wavelength
    
    # Store results
    mean_reflectance[cls_name] = means_full
    confidence_intervals[cls_name] = cis_full
...
for cls_name in mean_reflectance:
    means = mean_reflectance[cls_name]
    cis = confidence_intervals[cls_name]
    plt.plot(wavelengths_full, means, label=f'Class {cls_name}')
    plt.fill_between(wavelengths_full, 
                     np.array(means) - np.array(cis), 
                     np.array(means) + np.array(cis), 
                     alpha=0.2)

plt.xticks(wavelengths_full, labels=[f"{wl} nm" for wl in wavelengths_full])
plt.xlabel('Wavelength (nm)')
plt.ylabel('Reflectance')
plt.title('Mean Reflectance with Confidence Intervals Across Wavelengths')
plt.legend(title='Classes')
...
with rasterio.open(raster_path) as src:
    bands = src.read(out_dtype='float32')  # Shape: (5, height, width)
    bands[bands == src.nodata] = np.nan  # Handle nodata as NaN if applicable
    # Extract band-specific wavelengths from metadata if available (example assumption)
    wavelengths = src.tags().get("WAVELENGTHS", None)
    if wavelengths:
        wavelengths = list(map(float, wavelengths.split(',')))  # Convert to list of floats
    else:
        # If no metadata is available, manually define the wavelengths
        wavelengths = [475, 560, 668, 717, 842]  # Example values for 5 bands
...
# %%
###test 7 one plot for all directories
import os
import re
import numpy as np
import rasterio
